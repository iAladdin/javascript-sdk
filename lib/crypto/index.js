...
var decodeAddress = function decodeAddress(value) {
  var decodeAddressString = _bech["default"].decode(value);

  return Buffer.from(_bech["default"].fromWords(decodeAddressString.words));
};
/**
...
var checkAddress = function checkAddress(address) {
  try {
    var decodeAddressString = _bech["default"].decode(address);

    if (decodeAddressString.prefix === "tbnb" || decodeAddressString.prefix === "bnb") {
      return true;
    }
...
var generateKeyStore = function generateKeyStore(privateKeyHex, password) {
  var salt = (0, _randombytes["default"])(32);
  var iv = (0, _randombytes["default"])(16);
  ...
  var derivedKey = _crypto["default"].pbkdf2Sync(Buffer.from(password), salt, kdfparams.c, kdfparams.dklen, "sha256");

  var cipher = _crypto["default"].createCipheriv(cipherAlg, derivedKey.slice(0, 32), iv);
  ...
  return {
    version: 1,
    id: _uuid["default"].v4({
      random: (0, _randombytes["default"])(16)
    }),
    crypto: {
...
var getPrivateKeyFromKeyStore = function getPrivateKeyFromKeyStore(keystore, password) {
  ...
  var derivedKey = _crypto["default"].pbkdf2Sync(Buffer.from(password), Buffer.from(kdfparams.salt, "hex"), kdfparams.c, kdfparams.dklen, "sha256");
  ...
  var decipher = _crypto["default"].createDecipheriv(json.crypto.cipher, derivedKey.slice(0, 32), Buffer.from(json.crypto.cipherparams.iv, "hex"));
