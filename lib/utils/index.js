...
var ab2hexstring = function ab2hexstring(arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    str = str.length === 0 ? "00" : str.length === 1 ? "0".concat(str) : str;
    result += str;
  }
...
var num2VarInt = function num2VarInt(num) {
  ...
  if (num <= 0xffff) {
    // uint16
    return "fd".concat(num2hexstring(num, 2, true));
  }

  if (num <= 0xffffffff) {
    // uint32
    return "fe".concat(num2hexstring(num, 4, true));
  } // uint64


  return "ff".concat(num2hexstring(num, 8, true));
};
/**
 * XORs two hexstrings
